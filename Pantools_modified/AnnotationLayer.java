...
import static pantools.Pantools.qtl_label; // Lily
...
public class AnnotationLayer {
    ...
    /**
     * Implements a comparator for integer arrays of size three
     */
    ...
    public void add_annotations() {
        ...
        try{
            ...
            log_file = new BufferedWriter(new FileWriter(OUTPUT_PATH + "/annotation.log"));
            ...
            while ((line = paths.readLine()) != null) // for each gff file
            {
                line = line.trim();
                if (line.equals(""))
                    continue;
                ...
                try (Transaction tx = graphDb.beginTx()) {
                    ...
                    genome_node = graphDb.findNode(genome_label,"number", genome);
                    ...
                    annotation_node.createRelationshipTo(genome_node, RelTypes.annotates);
                        ...
                        if (annotation_file.endsWith(".gff") || annotation_file.endsWith(".gff3")){
                            parse_gff(genome, genome + "_" + degree, log_file, annotation_file, OUTPUT_PATH + "/proteins");
                            annotation_node.setProperty("type", "GFF");
                        }
                        else if (annotation_file.endsWith(".gbk") || annotation_file.endsWith(".gbff")){
                            parse_gbk(genome, genome + "_" + degree, log_file, annotation_file, OUTPUT_PATH + "/proteins");
                            ...
                            System.err.println("Genome " + genome + " does not exist!");
                    tx.success();
                }
            } // for genomes
            paths.close();
        ...
        try (Transaction tx = graphDb.beginTx()) {
            graphDb.findNodes(pangenome_label).next().setProperty("num_proteins", num_proteins);
            tx.success();
        }
     ...
     * @param annotation_file Path to the GFF3 file
     * @param proteins_directory The directory to write protein FASTA files in 
     */
    private void parse_gff(int genome, String annotation_id, BufferedWriter log_file, String annotation_file, String proteins_directory){
        int i, trsc, num_genes, num_mRNAs, num_tRNAs, num_rRNAs, num_QTLs, feature_len, offset; // Lily: add num_QTLs
        long seq_len = -1;
        ...
        address[0] = genome;
        num_genes = num_mRNAs = num_tRNAs = num_rRNAs = num_QTLs = 0; // Lily: initialize QTL count to zero
        try (BufferedReader in = new BufferedReader(new FileReader(annotation_file))) {
            ...
            while (in.ready()){
                try (Transaction tx2 = graphDb.beginTx()) {
                    for (trsc = 0; trsc < MAX_TRANSACTION_SIZE && (line = in.readLine()) != null; ++trsc) {
                        ...
                        if (fields[fields.length - 1].contains(";locus_tag=")){
                            String [] fields_split = fields[fields.length - 1].split(";locus_tag=");
                            String [] fields_split2 = fields_split[1].split(";");
                            feature_node.setProperty("locus_tag",  fields_split2[0]);
                        ...
                        parent_node = get_node_by_id(gene_nodes, rna_nodes, get_property(attribute,"Parent"));
                        if (parent_node != null)
                            ...
                            ++num_genes;
                                    ...
                                    case "CDS": 
                                        ...
                                        for (i=0;i<parent_ids.length;++i){
                                            rna_node = get_node_by_id(null,rna_nodes,parent_ids[i]);
                                            // for CDSs with a parent of type gene    
                                            if (rna_node == null){
                                                gene_node = get_node_by_id(gene_nodes,null,parent_ids[i]);
                                                if (gene_node != null){
                                    ...
                                    case "intron":
                                        ...
                                        break;
                                    // Lily: add a case to handle QTL features
                                    case "QTL":
                                        ++num_QTLs;
                                        feature_node.addLabel(qtl_label);
                                        feature_node.setProperty("trait", get_property(attribute,"Trait"));
                                        feature_node.setProperty("parent_a", get_property(attribute,"Parent_a"));
                                        feature_node.setProperty("parent_b", get_property(attribute,"Parent_b"));
                                        try{
                                            feature_node.setProperty("lod", Double.parseDouble(get_property(attribute,"LOD")));
                                        }
                                        catch(Exception e){
                                            feature_node.setProperty("lod", -1.0);
                                        }
                                        try{
                                            feature_node.setProperty("explained_variance", Double.parseDouble(get_property(attribute,"Explained_variance")));
                                        }
                                        catch(Exception e){
                                            feature_node.setProperty("explained_variance", -1.0);
                                        }
            ...
            set_protein_sequences(gene_nodes, address[0], log_file, proteins_directory);
            log_file.write("Genome "+address[0] + " : " + num_genes + " genes\t" + num_mRNAs + " mRNAs\t" + num_tRNAs + " tRNAs\t" + num_rRNAs + " rRNAs\n");
            log_file.write("----------------------------------------------------\n");
        }catch (IOException ioe) {
            System.out.println(ioe.getMessage());
            System.out.println("Could not open " + annotation_file + "!");
        }
    }
     ...
     * @param proteins_directory The directory to write protein FASTA files in 
     */
    private void parse_gbk(int genome, String annotation_id, BufferedWriter log_file, String annotation_file, String proteins_directory){
                ...
                try (Transaction tx2 = graphDb.beginTx()) {
                    for (trsc = 0; trsc < MAX_TRANSACTION_SIZE && (line = in.readLine()) != null; ++trsc) {
                        ...
                        if (line.startsWith("LOCUS")){
                            ...
                            switch (address[1]){
                                case -1:
                                    System.err.println("More than one sequence contain "+sequence_id+" in their ID in genome "+address[0]+"\n"); // usually organal genes
                                    System.exit(1);
                                    break;
                                case 0:
     ...
     * @param gene_nodes A list of annotated genes
     * @param rna_nodes A list of annotated mRNAs
     * @param id ID of the query feature
     * @return The annotation node (gene or mRNA) of the query feature
     */
    private Node get_node_by_id(LinkedList<feature> gene_nodes, LinkedList<feature> rna_nodes, String id){
        ListIterator<feature> itr;
        feature f;
        if (gene_nodes != null){
             itr = gene_nodes.listIterator();
            while (itr.hasNext()){
                f = itr.next();
     ...
     * @param log_file The log file of the annotation
     * @param proteins_directory The directory where protein FASTA files are written
     */
    private void set_protein_sequences(LinkedList<feature> gene_nodes, int genome, BufferedWriter log_file, String proteins_directory){
        ...
        try (BufferedWriter out = new BufferedWriter(new FileWriter(proteins_directory + "/proteins_" + genome + ".fasta"))) {
            ...
            while (!gene_nodes.isEmpty()){
                try (Transaction tx = graphDb.beginTx()) {
                    for (trsc = 0; !gene_nodes.isEmpty()&& trsc < 2 * MAX_TRANSACTION_SIZE; ++trsc){
                        ...
                        address[3] -= 1;
                        gene_builder.setLength(0);
                        genomeSc.get_sub_sequence(gene_builder, address[0], address[1], address[2], address[3] - address[2] + 1 , true);//(boolean)start_edge.getProperty("forward"));
                        ...
                        for (Relationship r1: gene_node.getRelationships(RelTypes.codes_for, Direction.OUTGOING)) {
                            ...
                            if (mrna_node.hasRelationship(RelTypes.contributes_to, Direction.INCOMING)){
                                ...
                                if (protein.length() > 0){
                                    ...
                                    mrna_node.setProperty("dna_sequence", rna_builder.toString());
                                    mrna_node.setProperty("protein_sequence", protein.toString());
    ...
    /**
     * Extracts the value of a given property from the attribute field 
     * 
    ...
    private int find_sequence(String name, int genome) {
        int i, sequence = 0, number_of_matches = 0;
        boolean found = false;
        for (i = 1; i <= genomeDb.num_sequences[genome] && !found; ++i){ 
            if (genomeDb.sequence_titles[genome][i].equals(name)){
                found = true;
                sequence = i;
            }
        }
        if (!found){
            for (i = 1; i <= genomeDb.num_sequences[genome]; ++i){
                //System.out.println(name + " -> " + genomeDb.sequence_titles[genome][i]);
            ...
            if (number_of_matches > 1){ // is not unique
                number_of_matches = 0;
                //System.out.println("to many hits!");
                for (i = 1; i <= genomeDb.num_sequences[genome]; ++i){
                    //System.out.println(name + " -> " + genomeDb.sequence_titles[genome][i]);
                    if (genomeDb.sequence_titles[genome][i].startsWith(name +" ")){
                        ++number_of_matches;
                ...
                if (number_of_matches > 1){ // is not unique
                    //System.out.println("still to many!");
                    sequence = -1;
                }
            }
        }        
        ...
        try (Transaction tx = graphDb.beginTx()) {
            try {
                ...
                for (i = 0; (line = in.readLine()) != null;) {
                    ...
                    out[genome] = new BufferedWriter(new FileWriter(OUTPUT_PATH + "/" + FEATURE + "s." + genome + ".fasta"));
                    ++i;
                }
                in.close();
                    ...
                    if (Arrays.binarySearch(genome_numbers, genome) >= 0){
                        ...
                        if (FEATURE == "mRNA")
                           ...
                           // start = rstart.getEndNode();
                            ...
                            address[2] -= 1;
                            address[3] -= 1;
                            genomeSc.get_sub_sequence(feature_seq, address[0], address[1], address[2], address[3] - address[2] + 1 , forward);
    ...
    /**
     * Removes an annotation from a genome
     */
